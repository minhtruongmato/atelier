...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
          ...
          @endif
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('kind.create') }}">Thêm mới nhóm sản phẩm</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
      ...
      <div class="row">
        <div class="col-sm-12">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                <th width="20%" class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column descending" aria-sort="ascending">Hình ảnh</th>
                <th width="20%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">Tên dòng sản phẩm</th>
            ...
            <tbody>
              @foreach($kinds as $value)
                <tr role="row" class="odd">
                  <td class="sorting_1">
                    {{ HTML::image('storage/app/kind/'.$value->image, '', array('width' => 150)) }}
                  </td>
                  <td>{{$value->title}}</td>
                  ...
                  @endif
                  <td>
                    <form class="row" method="POST" action="{{ route('kind.destroy', ['id' => $value->id]) }}" onsubmit = "return confirm('Chắc chắn xoá?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('kind.edit', ['id' => $value->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Sửa
                        </a>
